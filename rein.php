...
/**
 * Runs tests and outputs report
 *
 * This function will run all functions beginning with test_ found in all files
 * provided as arguments. Invalid files are skipped.
 *
 * @param string|array $files Either a string or an array of test files
 *
 * @return bool true if all tests passes, false otherwise
 */
function rein_run($files)
{
    $failures = array();
    ...
    assert_options(ASSERT_CALLBACK,   function($file, $line, $assert) use (&$failures) {
        $failures[] = array('file' => $file, 'line' => $line, 'assert' => $assert);
    });
